...
do 
	...
	musescore.exe -o "${noX}.mscz" $file
	musescore.exe -o "../PDF_Files/${noX}.pdf" $file
	rm $file
done
for file in ./*
do
	...
	then 
		...
		do
			...
			musescore.exe -o "${noX}.mscz" $subFile
			musescore.exe -o "../../PDF_Files/${file}/${noX}.pdf" $subFile
			rm $subFile
		done
		cd ..
	fi 
